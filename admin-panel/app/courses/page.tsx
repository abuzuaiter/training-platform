...
export default function CoursesPage() {
  ...
  async function loadData() {
    setLoading(true)
    const [cRes, oRes] = await Promise.all([fetch('/api/courses'), fetch('/api/organizations')])
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-900 font-semibold text-sm">Courses</span>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {courses.map(course => (
              <div key={course.id} className="bg-white rounded-2xl border border-gray-200 p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-amber-100 rounded-xl flex items-center justify-center text-2xl">📚</div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-gray-900">{course.name}</h3>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${enrollmentTypeColor[course.enrollment_type] || 'bg-gray-100 text-gray-500'}`}>
