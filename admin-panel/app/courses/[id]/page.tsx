...
export default function ManageCoursePage() {
  ...
  async function handleSave() {
    ...
    if (res.ok) { setMessage('Saved successfully!'); loadCourse() }
  ...
  async function handleAddSchedule() {
    ...
    const res = await fetch(`/api/courses/${id}/schedule`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  async function handleDeleteSchedule(scheduleId: string) {
    await fetch(`/api/courses/${id}/schedule/${scheduleId}`, { method: 'DELETE' })
    loadCourse()
  }

  if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">Loading...</div>
  if (!course) return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">Not found</div>
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <Link href="/courses" className="text-gray-400 hover:text-gray-600 text-sm">Courses</Link>
          <span className="text-gray-300">/</span>
          <span className="text-gray-900 font-semibold text-sm">{course.name}</span>
        </div>
      </nav>

      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">{course.name}</h1>
        <p className="text-sm text-gray-500">{course.organizations?.name}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          {course.course_schedule?.length === 0 ? (
            <p className="text-sm text-gray-400">No schedule yet</p>
          ) : (
            <div className="space-y-2">
              {course.course_schedule?.map(s => (
