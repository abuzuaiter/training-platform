...
export async function GET(req: NextRequest) {
  ...
  let query = supabaseAdmin.from('session_templates').select('*').order('created_at', { ascending: false })
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('session_templates').insert({
